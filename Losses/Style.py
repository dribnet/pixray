...
class StyleLoss(LossInterface):
    def __init__(self, device):
        ...
        self.feat_style = None
        for i in range(5):
            with torch.no_grad():
            # r is region of interest (mask)
                feat_e = extractor.forward_samples_hypercolumn(style, samps=1000)
                self.feat_style = feat_e if self.feat_style is None else torch.cat((self.feat_style, feat_e), dim=2)
    
    @staticmethod
    def add_settings(parser):
        parser.add_argument("--style_weight", type=float, help="how much symmetry is weighted in loss", default=1, dest='symmetry_weight')
        return parser
   
    def get_loss(self, cur_cutouts, out, args, globals=None, lossGlobals=None):
        print(out.shape)
        print(cur_cutouts)
        xx, xy = sample_indices(out, self.feat_style) # 0 to sample over first layer extracted
        styleloss = calculate_loss(out, out, self.feat_style, [xx, xy], 0)
        styleloss.backward()
        return styleloss
